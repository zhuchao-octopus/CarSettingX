...
public class FactorySettingsFragment extends PreferenceFragment implements Preference.OnPreferenceChangeListener, OnPreferenceClickListener {
    ...
    private final LogoFragment mLogoFragment = new LogoFragment();
    ...
    private void categorizePreferenceScreen() {
        if (mPerferenceMap.isEmpty()) {
            ...
            if (GeneralSettings.mSystemUI.equals(MachineConfig.VALUE_SYSTEM_UI20_RM10_1) || GeneralSettings.mSystemUI.equals(MachineConfig.VALUE_SYSTEM_UI21_RM10_2) || GeneralSettings.mSystemUI.equals(MachineConfig.VALUE_SYSTEM_UI21_RM12)) {
            ...
            if (isHideAddLanguage()) mPerferenceMap.put(KEY_ADD_LANGUAGE, FactorySettings.TAB_TYPE_OTHER);
            else mPerferenceMap.put(KEY_ADD_LANGUAGE, FactorySettings.TAB_TYPE_NONE);
            //			if (Util.isRKSystem())
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (mScrennSizePreference != null) {
            if ("1".equals(MachineConfig.getPropertyOnce(MachineConfig.KEY_OTG_TEST))) {
    ...
    private boolean isHideAddLanguage() {
        ...
        if (value != null && (value.equals("1") || value.equals("true"))) return true;
        else return false;
    }

    private void getModel() {
        String str = MachineConfig.getProperty(MachineConfig.KEY_MODEL);
        if (str == null || str.isEmpty()) str = Build.MODEL;
        mModel.setText(str);
...
//		String value = SystemConfig.getProperty(context, MachineConfig.KEY_TOUCH3_IDENTIFY);
    ...
    public void onResume() {
        ...
        mRds = !MachineConfig.VALUE_OFF.equals(MachineConfig.getProperty(MachineConfig.KEY_RDS));
        mRdsCheckbox.setChecked(mRds);

        int ant = MachineConfig.getPropertyInt(MachineConfig.KEY_RADIO_ANT_POWER);
        mRadioAntPowerCheckbox.setChecked(ant == 1 ? true : false);
        boolean no_reverse = MachineConfig.VALUE_ON.equals(MachineConfig.getProperty(MachineConfig.KEY_NO_REVERSE));
        mNoReverseCheckbox.setChecked(no_reverse);

        boolean rudder = MachineConfig.VALUE_ON.equals(MachineConfig.getProperty(MachineConfig.KEY_RUDDER));
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (KEY_ILLUM_ACC_NOD.equals(key)) {
            ...
            it.putExtra(MyCmd.EXTRA_COMMON_CMD, MachineConfig.KEY_PANEL_KEY_DEF_CONFIG);
            ...
            MachineConfig.setProperty(MachineConfig.KEY_RDS, mRds ? MachineConfig.VALUE_ON : MachineConfig.VALUE_OFF);

            Intent it = new Intent(MyCmd.BROADCAST_MACHINECONFIG_UPDATE);
            it.putExtra(MyCmd.EXTRA_COMMON_CMD, MachineConfig.KEY_RDS);
            mActivity.sendBroadcast(it);
            ...
            Intent it = new Intent(MyCmd.BROADCAST_MACHINECONFIG_UPDATE);
            it.putExtra(MyCmd.EXTRA_COMMON_CMD, MachineConfig.KEY_RADIO_ANT_POWER);
            it.putExtra(MyCmd.EXTRA_COMMON_DATA, ((Boolean) newValue) ? 1 : 0);
            mActivity.sendBroadcast(it);


            MachineConfig.setProperty(MachineConfig.KEY_RADIO_ANT_POWER, ((Boolean) newValue) ? MachineConfig.VALUE_ON : MachineConfig.VALUE_OFF);
            ...
            AlertDialog ad = new AlertDialog.Builder(mActivity).setTitle(R.string.reboot_warning).setPositiveButton(R.string.alert_dialog_ok, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {

                    MachineConfig.setProperty(MachineConfig.KEY_NO_REVERSE, no_reverse ? MachineConfig.VALUE_ON : MachineConfig.VALUE_OFF);

                    BroadcastUtil.sendToCarService(getActivity(), MyCmd.Cmd.APP_REQUEST_SEND_KEY, MyCmd.Keycode.POWER);
                    ...
                    Util.doSleep(1000);
                    Util.setFileValue("/sys/class/ak/source/factory", new byte[]{0x55, (byte) 0xaa, 0x00});
                }
            ...
            MachineConfig.setProperty(MachineConfig.KEY_RUDDER, (Boolean) newValue ? MachineConfig.VALUE_ON : MachineConfig.VALUE_OFF);
    ...
    public static void doBackup(final Activity activity) {
        AlertDialog ad = new AlertDialog.Builder(activity).setIcon(R.drawable.dialog_alert_icon).setTitle(R.string.backup_title).setMessage(R.string.backup_message).setPositiveButton(R.string.alert_dialog_ok, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
                SettingBackuper sb = new SettingBackuper(activity);
                ...
                if (sb.doBackup()) {
                    ...
                    Toast.makeText(activity, R.string.no_valid_ext_storage, Toast.LENGTH_SHORT).show();
                }
            }
        }).setNegativeButton(R.string.alert_dialog_cancel, null).create();
        ;
    ...
    public static void doRestore(final Activity activity) {
        AlertDialog ad = new AlertDialog.Builder(activity).setIcon(R.drawable.dialog_alert_icon).setTitle(R.string.restore_title).setMessage(R.string.restore_message).setPositiveButton(R.string.alert_dialog_ok, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
                SettingBackuper sb = new SettingBackuper(activity);
                if (sb.doRestore()) {
                    ...
                    AlertDialog adReboot = new AlertDialog.Builder(activity).setIcon(R.drawable.dialog_alert_icon).setTitle(R.string.restore_title).setMessage(R.string.reboot_warning).setPositiveButton(R.string.alert_dialog_i_know, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int whichButton) {
                    ...
                    Toast.makeText(activity, R.string.no_backuped_setting_found, Toast.LENGTH_SHORT).show();
                }
            }
        }).setNegativeButton(R.string.alert_dialog_cancel, null).create();
        ad.show();
    ...
    private void setBTMode(String value) {
        ...
        AlertDialog ad = new AlertDialog.Builder(mActivity).setTitle(R.string.reboot_warning).setPositiveButton(R.string.alert_dialog_ok, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
                ...
                MachineConfig.setProperty(MachineConfig.KEY_BT_TYPE, v);
    ...
    private void setTpms(final String value) {
        AlertDialog ad = new AlertDialog.Builder(mActivity).setTitle(R.string.reboot_warning).setPositiveButton(R.string.alert_dialog_ok, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
                MachineConfig.setProperty(MachineConfig.KEY_TPMS_TYPE, value);
                if (!value.equals("0")) {
                    // mutual exclusion with FTJ
                    MachineConfig.setProperty(MachineConfig.KEY_FTJ_TYPE, "0");
                }

                String appHide = MachineConfig.getProperty(MachineConfig.KEY_APP_HIDE);
                if ("2".equals(value)) {
                    if (appHide == null) {
                        appHide = MachineConfig.getPropertyReadOnly(MachineConfig.KEY_APP_HIDE);
                    }
    ...
    private void setFtj(final String value) {
        AlertDialog ad = new AlertDialog.Builder(mActivity).setTitle(R.string.reboot_warning).setPositiveButton(R.string.alert_dialog_ok, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int whichButton) {
                MachineConfig.setProperty(MachineConfig.KEY_FTJ_TYPE, value);
                if (!value.equals("0")) {
                    // mutual exclusion with TPMS
                    MachineConfig.setProperty(MachineConfig.KEY_TPMS_TYPE, "0");
                }
                BroadcastUtil.sendToCarService(getActivity(), MyCmd.Cmd.APP_REQUEST_SEND_KEY, MyCmd.Keycode.POWER);
    ...
    private void setScreenSize(String value) {
        ...
        if (w != null && h != null) {
            if (!w.equals(old_w) || !h.equals(old_h)) {
                ...
                AlertDialog ad = new AlertDialog.Builder(mActivity).setTitle(R.string.screen_size_reboot).setPositiveButton(R.string.alert_dialog_ok, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {

                        MachineConfig.setProperty(MachineConfig.KEY_SCREEN_W, w);
                        MachineConfig.setProperty(MachineConfig.KEY_SCREEN_H, h);
                        Util.doSleep(300);
    ...
    private void setAppHideSetting(HashSet<String> which) {
        ...
        String tpms = MachineConfig.getProperty(MachineConfig.KEY_TPMS_TYPE);
        if ("2".equals(tpms)) {
